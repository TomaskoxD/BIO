...
class ResNet18_v0(nn.Module):
    ...
    """
    def __init__(self, n_class):
        super(ResNet18_v0, self).__init__()
...
class resnet18_mcs(nn.Module):
    ...
    def __init__(self, n_class):
        super(resnet18_mcs, self).__init__()

        A_model = ResNet18_v0(n_class=n_class)
        self.featureA = A_model
        self.classA = nn.Sequential(*list(A_model.resnet18.children())[:-1])

        B_model = ResNet18_v0(n_class=n_class)
        self.featureB = B_model
        self.classB = nn.Sequential(*list(B_model.resnet18.children())[:-1])

        C_model = ResNet18_v0(n_class=n_class)
        self.featureC = C_model
        self.classC = nn.Sequential(*list(C_model.resnet18.children())[:-1])

        num_ftrs = A_model.resnet18.layer4[1].bn2.num_features
